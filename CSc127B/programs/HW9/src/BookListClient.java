...
public class BookListClient {
	public static void main(String[] args) throws FileNotFoundException {
		...
		// Call BookList Constructor passing in the input file
		myBooks = new BookList(fileName);

		// Print the books currently in the inventory, including authors
		...
		while (commandsInput.hasNext()) {
			...
			else if (line.length() >= 14 && line.substring(0, 14).equals("TITLE-AUTHORS ")) {
			...
			// Process ADD
			else if (line.length() >= 4 && line.substring(0, 4).equals("ADD ")) {
			...
			else if (line.length() >= 13 && line.substring(0, 13).equals("DELETE-TITLE ")) {
			...
			else if (line.length() >= 20 && line.substring(0, 20).equals("PRINT-TITLES-AUTHORS")) {
				System.out.println("PRINT-TITLES-AUTHORS");
				System.out.println(myBooks.toString());
			}

			// Process PRINT-TITLES
			else if (line.length() >= 12 && line.substring(0, 12).equals("PRINT-TITLES")) {
				System.out.println("PRINT-TITLES");
				System.out.println(myBooks.toStringTitles());
			}

			// Process TITLE-ADD-AUTHOR
			else if (line.length() >= 17 && line.substring(0, 17).equals("TITLE-ADD-AUTHOR ")) {
				...
				if (currentTitle == null)
					...
					System.out.printf("ADDED AUTHOR %s", author);
				}
			}
		}
