# ----------- main() -----------
.text


.globl	main
main:
	# fill the sX registers (and fp) with junk.  Each testcase will use a different
	# set of values.
	lui   $fp,      0xffff
	ori   $fp, $fp, 0xfff6
	lui   $s0,      0x0000
	ori   $s0, $s0, 0x0075
	lui   $s1,      0xffff
	ori   $s1, $s1, 0xfff7
	lui   $s2,      0x0000
	ori   $s2, $s2, 0x0000
	lui   $s3,      0x000c
	ori   $s3, $s3, 0xbbd2
	lui   $s4,      0xffff
	ori   $s4, $s4, 0xfc4b
	lui   $s5,      0xffff
	ori   $s5, $s5, 0xfd28
	lui   $s6,      0x0000
	ori   $s6, $s6, 0xa763
	lui   $s7,      0x0000
	ori   $s7, $s7, 0x0186

	# instead of explicitly dumping the stack pointer, I'll push a dummy
	# variable onto the stack.  Some students are reporting different
	# stack values in their output.
	lui   $t0, 0xabcd
	ori   $t0, $t0,0x1234
	addiu $sp, $sp,-4
	sw    $t0, 0($sp)


.data
STR1:	.asciiz	"asdf"
STR2:	.asciiz	""
STR3:	.asciiz	"\n"
STR4:	.asciiz	"The quick brown fox jumps over the lazy dog."
STR5:	.asciiz "A null terminator in the string constant\0 has strange effects.\n"
.text
	# strlen(STR1)
	la      $a0, STR1
	jal     strlen
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# strlen(STR2)
	la      $a0, STR2
	jal     strlen
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# strlen(STR3)
	la      $a0, STR3
	jal     strlen
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# strlen(STR4)
	la      $a0, STR4
	jal     strlen
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# strlen(STR5)
	la      $a0, STR5
	jal     strlen
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall
	
	
	# multiply(13,15)
	addi    $a0, $zero,13
	addi    $a1, $zero,15
	jal     multiply
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# multiply(-2,-3)
	addi    $a0, $zero,-2
	addi    $a1, $zero,-3
	jal     multiply
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall


	# multiply4(2,3,5,7)
	addi    $a0, $zero,2
	addi    $a1, $zero,3
	addi    $a2, $zero,5
	addi    $a3, $zero,7
	jal     multiply4
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

	# multiply8(2,-3,5,-7,7,-2,-5,-3)
	addi    $a0, $zero,2
	addi    $a1, $zero,-3
	addi    $a2, $zero,5
	addi    $a3, $zero,-7

	addi    $t0, $zero,7       # arg5
	sw      $t0, -16($sp)

	addi    $t0, $zero,-2      # arg6
	sw      $t0, -12($sp)

	addi    $t0, $zero,-5      # arg7
	sw      $t0, -8($sp)

	addi    $t0, $zero,-3      # arg8
	sw      $t0, -4($sp)

	jal     multiply8          # make the call!
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall

.data
INTS:	.word	0
	.word   1
	.word   2
	.word   3
	.word   4
	.word   5
	.word   6
	.word   7
	.word	8
	.word   9
	.word   10
	.word   11
	.word   12
	.word   13
	.word   14
	.word   15
.text
	# sumN(INTS, 16)
	la      $a0, INTS      # arg1 = &INTS[0]
	addi    $a1, $zero,16  # arg2 = 16
	jal     sumN
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall
	
	# sumN(INTS, 15)
	la      $a0, INTS      # arg1 = &INTS[0]
	addi    $a1, $zero,15  # arg2 = 15
	jal     sumN
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall
	
	# sumN(INTS+10, 3)
	la      $a0, INTS      # arg1 = &INTS[0]
	addi    $a0, $a0,40    # arg1 = &INTS[10]
	addi    $a1, $zero,3   # arg2 = 3
	jal     sumN
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall
	
	
.data
PWRS:	.word	1         # 2^0 - that is, the 1st element
	.word   2
	.word   4
	.word   8
	.word   16
	.word   32
	.word   64
	.word   128
	.word	256
	.word   512
	.word   1024
	.word   2048
	.word   4096
	.word   8192
	.word   16384
	.word   32768     # 2^15  - that is, the 16th element
.text
	# sumN(PWRS, 16)
	la      $a0, PWRS      # arg1 = &PWRS[0]
	addi    $a1, $zero,16  # arg2 = 16
	jal     sumN
	
	add     $a0, $v0,$zero            # print_int(retval)
	addi    $v0, $zero,1
	syscall
	
	addi    $v0, $zero,11             # print_char('\n')
	addi    $a0, $zero,'\n'
	syscall
	

#.data
#CAT1:	.asciiz	"First"
#.text
#	la      $a0, CAT1
#	la      $a1, CAT2
#	jal     concatAndPrint
#
#.data
#CAT3:	.asciiz	"XYZ"
#CAT4:	.asciiz	"abc"
#.text
#	la      $a0, CAT4
#	la      $a1, CAT3
#	jal     concatAndPrint

	
	# dump out all of the registers.
.data
TESTCASE_DUMP1:	.ascii  "\n"
               	.ascii 	"+-----------------------------------------------------------+\n"
		.ascii	"|    Magic Value (popped from the stack):                   |\n"
		.asciiz	"+-----------------------------------------------------------+\n"

TESTCASE_DUMP2:	.ascii  "\n"
               	.ascii 	"+-----------------------------------------------------------+\n"
		.ascii	"|    Testcase Register Dump (fp, then 8 sX regs):           |\n"
		.asciiz	"+-----------------------------------------------------------+\n"
.text
	addi	$v0, $zero, 4		# print_str(TESTCASE_DUMP1)
	la	$a0, TESTCASE_DUMP1
	syscall

	# we pop this from the stack so that, if the stack pointer is not
	# predictable, we'll still get reliable results.
	lw      $a0, 0($sp)
	addiu   $sp, $sp,4
	jal     printHex

	# the rest of the registers have hard-coded values
	addi	$v0, $zero, 4		# print_str(TESTCASE_DUMP2)
	la	$a0, TESTCASE_DUMP2
	syscall

	add	$a0, $fp, $zero
	jal     printHex
	add	$a0, $s0, $zero
	jal     printHex
	add	$a0, $s1, $zero
	jal     printHex
	add	$a0, $s2, $zero
	jal     printHex
	add	$a0, $s3, $zero
	jal     printHex
	add	$a0, $s4, $zero
	jal     printHex
	add	$a0, $s5, $zero
	jal     printHex
	add	$a0, $s6, $zero
	jal     printHex
	add	$a0, $s7, $zero
	jal     printHex
	
	# terminate the program	
	addi	$v0, $zero, 10		# syscall_exit
	syscall
	# never get here!
	
	
	
.globl sumN_DEBUG
sumN_DEBUG:
	# standard prologue
	addiu  $sp, $sp, -24
	sw     $fp, 0($sp)
	sw     $ra, 4($sp)
	sw     $a0, 8($sp)       # save arg1 for later
	sw     $a1, 12($sp)      # save arg2 for later
	addiu  $fp, $sp, 20
	
.data
sumN_DEBUG_MSG1:	.asciiz	"sumN_DEBUG: len="
sumN_DEBUG_MSG2:	.asciiz	" val="
.text

	addi   $v0, $zero,4         # print_str(MSG1)
	la     $a0, sumN_DEBUG_MSG1
	syscall
	
	addi   $v0, $zero,1         # print_int(len)
	lw     $a0, 8($sp)
	syscall
	
	addi   $v0, $zero,4         # print_str(MSG2)
	la     $a0, sumN_DEBUG_MSG2
	syscall

	addi   $v0, $zero,1         # print_int(val)
	lw     $a0, 12($sp)
	syscall
	
	addi   $v0, $zero,11        # print_char('\n')
	addi   $a0, $zero,'\n'
	syscall
	
	# standard epilogue
	lw     $ra, 4($sp)
	lw     $fp, 0($sp)
	addiu  $sp, $sp, 24
	jr     $ra



.globl printHelper
printHelper:
	# standard prologue
	addiu  $sp, $sp, -24
	sw     $fp, 0($sp)
	sw     $ra, 4($sp)
	sw     $a0, 8($sp)      # save a0, for later use
	addiu  $fp, $sp, 20
	
.data
printHelper_MSG1:	.asciiz "The string you wanted to print was: '"
printHelper_MSG2:	.asciiz "'\n"

printHelper_ERRMSG:	.asciiz "ERROR: The pointer you sent to printHelper() was not on the stack!\n"
.text
	srl    $t0, $a0,16         # t0 = (arg1 >> 16)
	ori    $t1, $zero,0x7fff   # t1 = 0x7fff
	beq    $t0,$t1, printHelper_OK
	
	# if we get here, then the user sent us a pointer to something other than the stack!
	addi   $v0, $zero,4        # print_str(ERRMSG)
	la     $a0, printHelper_ERRMSG
	syscall
	
	j      printHelper_DONE
	
printHelper_OK:
	# if we get here, then the string seems OK, and we'll print it.
	addi   $v0, $zero,4        # print_str(MSG1)
	la     $a0, printHelper_MSG1
	syscall
	
	# print_str(arg1)  -  no need to change $v0
	lw     $a0, 8($sp)
	syscall
	
	# print_str(MSG2)  -  no need to change $v0
	la     $a0, printHelper_MSG2
	syscall
	
printHelper_DONE:
	# standard epilogue
	lw     $ra, 4($sp)
	lw     $fp, 0($sp)
	addiu  $sp, $sp, 24
	jr     $ra



# void printHex(int val)
# {
#     printHex_recurse(val, 8);
#     printf("\n");
# }
printHex:
	# standard prologue
	addiu  $sp, $sp, -24
	sw     $fp, 0($sp)
	sw     $ra, 4($sp)
	addiu  $fp, $sp, 20
	
	# printHex(val, 8)
	addi   $a1, $zero, 8
	jal    printHex_recurse
	
	addi   $v0, $zero, 11      # print_char('\n')
	addi   $a0, $zero, 0xa
	syscall

	# standard epilogue
	lw     $ra, 4($sp)
	lw     $fp, 0($sp)
	addiu  $sp, $sp, 24
	jr     $ra
	
	
	
printHex_recurse:
	# standard prologue
	addiu  $sp, $sp, -24
	sw     $fp, 0($sp)
	sw     $ra, 4($sp)
	addiu  $fp, $sp, 20
	
	# if (len == 0) return;    // base case (NOP)
	beq    $a1, $zero, printHex_recurse_DONE

	# recurse first, before we print this character.
	#
	# The reason for this is because the easiest way to break up
	# a long integer is using a small shift and modulo; so *this*
	# call will be responsible for the *last* hex digit, and we'll
	# use recursion to handle the things which come *before* it.
	#
	# As we've seen just above, if the current len==1, then the
	# recursive call will be the base case, and a NOP.
	
	# of course, we have to save a0 before we recurse.  We do *NOT*
	# need to save a1, since we'll never need it again.
	sw     $a0, 8($sp)
	
	# printHex_recurse(val >> 4, len-1)
	srl    $a0, $a0,4
	addi   $a1, $a1,-1
	jal    printHex_recurse
	
	# restore a0
	lw     $a0, 8($sp)
	
	# the value we will print is (val & 0xf), interpreted as hex.
	andi   $t0, $a0,0x0f      # digit = (val & 0xf)
	
	slti   $t1, $t0,10        # t1 = (digit < 10)
	beq    $t1, $zero, printHex_recurse_LETTER
	
	# if we get here, then $t0 contains an integer from 0 to 9, inclusive.
	addi   $v0, $zero, 11     # print_char(digit+'0')
	addi   $a0, $t0, '0'
	syscall
	
	j      printHex_recurse_DONE
	
printHex_recurse_LETTER:
	# if we get here, then $t0 contains an integer from 10 to 15, inclusive.
	# convert to the equivalent letter.
	addi   $t0, $t0,-10        # digit -= 10
	
	addi   $v0, $zero, 11     # print_char(digit+'a')
	addi   $a0, $t0, 'a'
	syscall
	
	# intentional fall-through to the epilogue	

printHex_recurse_DONE:
	# standard epilogue
	lw     $ra, 4($sp)
	lw     $fp, 0($sp)
	addiu  $sp, $sp, 24
	jr     $ra

