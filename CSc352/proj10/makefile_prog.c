...
int main (int argc, char *argv[]) {
        ...
        if (argc == 1) {
                fileName = "myMakefile";
                target = "";//fist target defined in makefile
        }
        else if (argc == 2) {
                fileName = "myMakefile";
                if (strcmp(file_dash,argv[1]) == 0) {
                        fprintf(stderr,"Usage: ex_mymake2 [-f makefile] [target]\n");
                        exit(1);
                }
                else {
                        target = argv[1];
                ...
                else if (strcmp(file_dash,argv[2]) == 0) {
                        fileName = argv[3];
                        target = argv[1];
                }
			...
			if (strcmp(target,"") == 0) {
				target = firstTarget->name;
                        ...
                        char* findLine = target;
                        findLine = trimwhitespace(findLine);
                        if (nodeTarget(findLine) != 0) {
                                printTraversal(findLine);
                                free_list();
                        }
                        else {
                                free_list();
