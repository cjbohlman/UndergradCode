...
void jacobi(float*, float*,int, int, int);
...
int main ( int argc, char** argv )
{
	...
	for (x = 0; x < 5; x++) {
		for (y = 0; y < 32; y++) {
			char buf[20];
                        sprintf(buf, "sssem: %d %d", x,y);
			sem_unlink(buf);
			
			stageArr[x][y] = sem_open(buf, O_CREAT, 0700, 0);
	...
	FILE *fptr;
	fptr = fopen("JacobiResults.txt", "w");
	if (fptr == NULL)
    	{
